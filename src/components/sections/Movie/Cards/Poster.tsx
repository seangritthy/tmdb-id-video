...
const MoviePosterCard: React.FC<MoviePosterCardProps> = ({ movie, variant = "full", href, disableHover = false }) => {
  const { hovered, ref } = useHover();
  const [opened, handlers] = useDisclosure(false);
  const parsedYear = new Date(movie.release_date).getFullYear();
  const releaseYear = Number.isNaN(parsedYear) ? "" : parsedYear;
  ...
  const finalHref = href || `/movie/${movie.id}`;

  const cardContent = (
    <Link href={finalHref} ref={ref} {...longPress()}>
      {variant === "full" && (
        <div className="group motion-preset-focus relative aspect-2/3 overflow-hidden rounded-lg border-[3px] border-transparent text-white transition-colors hover:border-primary">
              {(hovered || mobile) && (
                <Icon
                  ...
                  width="64"
                  height="64"
                  className="absolute-center z-20"
                  style={{ color: "url(#googlePlayGradient)" }}
                />
              )}
              {movie.adult && (
                <Chip
