    ...
    >
      ...
      <div className="relative flex flex-col gap-2">
        <div className="flex items-center justify-center gap-2">
          <SearchInput
            autoFocus
            placeholder={content === "movie" ? dictionary.searchPlaceholderMovies : dictionary.searchPlaceholderTvShows}
            isLoading={isLoading}
            value={searchQuery}
            onValueChange={(val) => {
              setSearchQuery(val);
          ...
          <AnimatePresence>
            {!isEmpty(searchQuery) && (
              ...
              >
                ...
                >
                  {mobile ? <Search /> : dictionary.searchButton}
                </Button>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        {(showSuggestions || showHistory) && !(showSuggestions && isEmpty(data?.data)) && (
          <AnimatePresence>
            ...
            >
              <Listbox
                variant="flat"
                emptyContent={<p className="text-center">{dictionary.noSearchSuggestions}</p>}
