...
const TvShowHomeList: React.FC<QueryList<TV>> = ({ query, name, param }) => {
  ...
  // Find the matching key in the dictionary based on the param or fallback to name
  const titleKey = `${param}TvShows` as TranslationKey;
  const translatedName = dictionary[titleKey] || name;
    ...
    <section id={key} className="min-h-[250px] md:min-h-[300px]" ref={ref}>
      {isPending ? (
        ...
        <div className="z-3 flex flex-col gap-2">
          <div className="flex grow items-center justify-between">
            <SectionTitle color="warning">{translatedName as React.ReactNode}</SectionTitle>
            <Link
              size="sm"
